...
function ChatRoom({ charId, onBack }) {
    ...
    return (
        <div className="relative flex flex-col h-full bg-[#fdf6e3]">
            {/* Header */}
            ...
            {/* Input - Absolute within container, above navbar */}
            <div className="absolute bottom-0 left-0 right-0 p-3 bg-white border-t-4 border-black z-20">
                <div className="flex gap-2">
                    <input
                        ...
                        placeholder={`說點什麼...`}
                        className="flex-1 bg-gray-100 border-2 border-black px-4 py-2 focus:outline-none focus:bg-white transition-all text-xs font-mono"
                    />
                    <button
                        onClick={handleSend}
                        disabled={loading}
                        className="bg-amber-500 text-black border-2 border-black px-3 py-2 hover:bg-amber-400 disabled:opacity-50 transition-none active:translate-y-1 active:shadow-none shadow-[2px_2px_0_0_#000]"
                    >
                        <Send size={16} />
                    </button>
                </div>
